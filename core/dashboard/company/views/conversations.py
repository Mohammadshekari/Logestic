...
class ConversationListView( LoginRequiredMixin, HasCompanyAccess, ListView):
    template_name = "dashboard/company/conversations/conversation-list.html"
    paginate_by = 10
    ordering = "-created_date"

    def get_queryset(self):
        messages_queryset = OfferMessageModel.objects.filter(
            offer__user=self.request.user).order_by("-created_date").values_list("offer")
        queryset = OfferModel.objects.filter(
            id__in=messages_queryset).order_by("-created_date")
